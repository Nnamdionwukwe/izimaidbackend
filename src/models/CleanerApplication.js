...
class CleanerApplication {
  static async create(applicationData) {
    ...
    try {
      ...
      console.error("Error creating cleaner application:", error);
      throw error;
    }
  }
  ...
  static async findAll(filters = {}) {
    ...
    let countQuery = `
      SELECT COUNT(*) FROM cleaner_applications WHERE 1=1
    `;
    if (conditions.length) {
  ...
  static async findById(id) {
    ...
    try {
      ...
      console.error("Error finding cleaner application by ID:", error);
      throw error;
    }
  }
  ...
  static async findByEmail(email) {
    const query = `
      SELECT * FROM cleaner_applications WHERE email = $1
    ...
    try {
      ...
      console.error("Error finding cleaner application by email:", error);
      throw error;
    }
  }
  ...
  static async updateStatus(id, status, adminNotes = null, reviewedBy = null) {
    ...
    try {
      ...
      console.error("Error updating cleaner application status:", error);
      throw error;
    }
  }
  ...
  static async delete(id) {
    const query = `DELETE FROM cleaner_applications WHERE id = $1 RETURNING *`;

    try {
      const { rows } = await pool.query(query, [id]);
      return rows[0] || null;
    } catch (error) {
      console.error("Error deleting cleaner application:", error);
      throw error;
    }
  }
  ...
  static async getStats() {
    ...
    try {
      ...
      console.error("Error getting cleaner application stats:", error);
      throw error;
    }
  }
}
